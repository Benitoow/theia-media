...
| G12c | Manual | Start a film. Click once on the picture — which is how a person pauses and resumes — then leave the pointer alone and watch the control bar. | 10 s | The furniture goes after three seconds, exactly as it does without the click. **Measured on 16 September 2026 and it does not:** with a 180 s film playing, a single click on the picture, and nine seconds of no further input, the bar was still up in two screenshots three seconds apart. The same build, started and left alone, hides it after three seconds. In Chromium the same click passes the render check, so this is a WebView2 difference and the simulation cannot see it. Two causes are plausible and neither is established: WebView2 may deliver a mouse move of its own while the click is pressed, or it may focus an element inside the furniture in a way the check does not reproduce. Recorded rather than worked around. |
