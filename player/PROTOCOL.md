...
| G12 | Simulation + Manual | Trigger the audio-fallback notice (a film whose bitstream the endpoint refuses), then wait. | at 5 s, then 12 s | The notice is still there at 5 s - it stays long enough to read, and the idle timer does not sweep it away - and it is gone by 12 s. Leaving the film clears it immediately, so it never follows the viewer back to the library. Asserted by `assertNoticeLivesWithItsState`; the manual half is the same gesture on a real film. |
...
| G19 | Simulation + Manual | Play a film, put the pointer on the volume icon, then drag the bar to the middle, to the end and back. | — | The bar expands under the pointer, the level follows the thumb instead of jumping back to where it was, and at the end of the track the film is silent. Moving the pointer away closes it again. Asserted by `assertVolume`; the manual half is the same gesture on a real film. |
| G20 | Simulation + Manual | With a film playing and nothing focused, press `↑` and `↓`. | — | The volume moves by 10% per press and the film does not seek. In the library the same keys belong to the row that has focus, so nothing moves the sound there. Asserted. |
| G20b | Simulation + Manual | Open Réglages, then **Visionnage**. | — | The section reports what was watched **for the profile this player is set to** - hours of films and of series, started and watched counts, the films/series split, and the series ranked by time watched - and those numbers are the server's, not the player's. The section is a dashboard: an overview of one tile per number (total time, films/series/episodes watched, completion rate), the calendar month on one line, the films/series ratio as a split bar, and the ranking - each ranked series with its TMDB poster and a rule showing how far into it the viewer is. Two tiles of the reference are absent on purpose: a per-session average and a streak need to know *when* things were watched, and the schema keeps only the last report per item. The ratio bar is the `partition-bar` component the maintainer brought, kept as it arrived; the bars stop animating when the reduced-motion preference is on. A server without the route says so in the active language instead of drawing zeroes, and a library nobody has watched yet is one sentence rather than an empty ranking. Asserted by the render check's settings journey; the manual half is the same section on the real library. |
| G21 | Simulation + Manual | Play a film, open the menu, press `→` to reach the Qualité tab, choose `720p`, then choose « Qualité du fichier ». | — | The tick moves, the film comes back at the second it left - the clock does not restart - and the sound returns to the file's own track, untouched. Asserted by `assertQualityTabs`; the manual half is the same gesture on a real film, where the picture visibly changes and the server log names a converted stream. |
| G22 | Simulation | With every encoder slot taken (`transcode.busy`), open the Qualité tab. | — | The rungs that need an encoder are greyed and cannot be pressed; the file's own rung stays pressable, because it needs none. Asserted. |
| G23 | Simulation + Manual | With a film playing **at a rung**, drag the scrub bar and press the skip buttons. | — | The film moves by reloading the converted stream at the new position, not by seeking a pipe that has no ranges: a second of black, then the film at the time that was asked for. The clock keeps the film's time - the `t=` plus what the engine counts - and the resume point written to the server is the film's, not the stream's. |
| G24 | Simulation + Manual | Choose a rung, leave the film, then start another one. | — | The new film starts at the file's own quality: a rung belongs to the film it was chosen for. |
| G25 | Simulation + Manual | Open Réglages, then **Sous-titres**. Move both sliders, then pick a colour, an outline, a band, a font and a weight, and watch the preview. | — | Every choice shows in the preview at the film's own proportion: the sample's size and its distance from the bottom are the numbers the sliders show, measured against a picture 1080 tall. **Tout réinitialiser** is grey until something has changed and puts all seven back; saving hands the engine the same object the page stored. Asserted by the render check, which reads the arithmetic out of the page rather than photographing it; the manual half is the same sheet on a real film, where the subtitles take the size, the colour, the outline and the place. |
| G26 | Simulation + Manual | Play an episode with the autoplay row of **Lecture** on, and let it end. | at the end | The next episode starts by itself, at its own resume point, with its own subtitle files, and the film's progress is written to the server as usual. With the row off, the last frame freezes as G25b of the full-screen section describes and nothing moves. |
| G27 | Manual | Play a film whose subtitles are embedded as an ASS script carrying its own margins - most disc remuxes - and move **Hauteur**. | — | The dialogue follows the slider, and a line the script deliberately places elsewhere (a sign, a title) keeps its side of the picture. Measured headlessly on the pinned engine on 24 September 2026, where `sub-margin-y` was silently ignored on that kind of track and `sub-pos` was not; the manual half is the film over the maintainer's own library. |
