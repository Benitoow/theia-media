...
func TestHomeHeroPrefersWhatIsAlreadyUnderway(t *testing.T) {
	...
	movies, err := service.store.List(t.Context(), testProfileID, 10, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	home, err := service.HomeScreen(t.Context(), testProfileID, 12)
	if err != nil {
		t.Fatal(err)
	}
	...
	started := movies[1].ID
	if _, err := service.SaveProgress(t.Context(), testProfileID, started, 1800, 10140); err != nil {
		t.Fatal(err)
	}
...
func TestTonightHoldsForTheEveningAndTurnsOver(t *testing.T) {
	...
	movies, err := service.store.List(t.Context(), testProfileID, 50, 0)
	if err != nil {
		t.Fatal(err)
	}
	for _, m := range movies {
	...
	order := func(seed int64) string {
		picks, err := service.store.Tonight(t.Context(), testProfileID, 12, seed)
		if err != nil {
			t.Fatal(err)
		}
...
func TestTonightIsNotAnArithmeticProgression(t *testing.T) {
	...
	movies, err := service.store.List(t.Context(), testProfileID, 100, 0)
	if err != nil {
		t.Fatal(err)
	}
	for _, m := range movies {
	...
	picks, err := service.store.Tonight(t.Context(), testProfileID, 12, 20260729)
	if err != nil {
		t.Fatal(err)
	}
	if len(picks) < 3 {
...
func TestTonightLeavesOutWhatIsAlreadyFinished(t *testing.T) {
	...
	if _, err := service.SaveProgress(t.Context(), testProfileID, finished, 10100, 10140); err != nil {
		t.Fatal(err)
	}

	picks, err := service.store.Tonight(t.Context(), testProfileID, 12, 1)
	if err != nil {
		t.Fatal(err)
	}
	for _, p := range picks {
